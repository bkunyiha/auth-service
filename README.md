...
mkdir -p mount_dir/auth-service/html/.well-known/acme-challenge
```
...
4. **ACME challenge connection refused**: If you get `Connection refused` when fetching ACME challenge files, the web root directory structure is missing. Fix with:
   ```bash
   mkdir -p ./mount_dir/auth-service/html/.well-known/acme-challenge
